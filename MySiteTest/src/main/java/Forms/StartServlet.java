...
public class StartServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (cookies != null) {
			for (int i = 0; i < cookies.length && check == false; i++) {
				if (cookies[i].getName().equals("email") && cookies[i + 1].getName().equals("password")) {
					...
					if (person.checkEmail() && person.checkPassword() && personIn!=null) {
		...
		if (check == false) {
			RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/views/Start.jsp");
			dispatcher.forward(request, response);
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (email != null && pass != null) {
			...
			if (person.checkEmail() && person.checkPassword() && personIn!=null) {
				...
				RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/views/Main.jsp");
				dispatcher.forward(request, response);
			} else{ RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/views/Start.jsp");
			       dispatcher.forward(request, response);
			}
